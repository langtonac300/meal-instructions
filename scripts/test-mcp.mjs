...
async function runTests() {
  ...
  const cookTimeData = JSON.parse(cookTimeRes.content[0].text);
  ...
  const recipeData = JSON.parse(recipeRes.content[0].text);
  ...
  console.log(`✅ search_recipes found ${searchData.totalMatches} fast beef recipe(s): ${searchData.recipes.map((r) => r.title).join(', ')}`);
  ...
  const reviveData = JSON.parse(reviveRes.content[0].text);
  ...
  const frozenData = JSON.parse(frozenRes.content[0].text);
  ...
  const meatMathData = JSON.parse(meatMathRes.content[0].text);
  ...
  const pullTempData = JSON.parse(pullTempRes.content[0].text);
  ...
  const troubleData = JSON.parse(troubleRes.content[0].text);
