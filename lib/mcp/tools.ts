/**
 * The single definition of the Meal Instructions MCP tool surface.
 *
 * There used to be three: an SDK server in this directory, a standalone
 * JSON-RPC handler in app/api/mcp/route.ts, and a static card in
 * app/.well-known/mcp/server-card.json/route.ts. Each carried its own copy of
 * every tool name, description and input schema, so each drifted separately —
 * "70 curated recipes" was wrong by 158 in all three at once, `boiling` was a
 * live appliance that two of them never mentioned, and the two handler sets had
 * quietly diverged in behaviour: the HTTP endpoint matched on a single
 * `includes()` where the SDK server did multi-field matching with a fallback.
 *
 * Now every transport reads from here.
 *
 * - `TOOL_DEFINITIONS` holds the zod schema and the handler for each tool.
 * - `toolListPayload()` derives JSON Schema from those same zod shapes via
 *   zod 4's native `z.toJSONSchema()`, so the advertised schema cannot drift
 *   from the validated one.
 * - `executeTool()` is the one place a tool actually runs.
 *
 * Adding a tool here adds it to stdio, HTTP and the server card at once.
 */
import { z } from 'zod';
...
export interface ToolDefinition {
  ...
  /**
   * `any` is the escape hatch a heterogeneous registry needs: each tool has a
   * different argument type, so the stored array cannot be typed precisely.
   * Type safety is preserved where it matters — `defineTool` below is generic
   * over the zod shape, so every handler body is checked against its own schema.
   */
...
/**
 * Registers one tool. Deliberately takes the same four arguments in the same
 * order as the MCP SDK's `server.tool()`, which is what let the existing
 * handlers move here unchanged rather than being retyped.
 */
function defineTool<S extends z.ZodRawShape>(
  ...
  TOOL_DEFINITIONS.push({ name, description, schema, handler });
}
...
/**
 * The tool list in JSON Schema form, for the HTTP endpoint's `tools/list` and
 * for the server card. Derived from the same zod shapes the SDK server
 * validates against — `$schema` is stripped because MCP clients expect a bare
 * schema object there.
 */
export function toolListPayload() {
  return TOOL_DEFINITIONS.map((t) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    // `io: 'input'` matters: several tools use `.default(...)`, and zod's default
    // output view marks those fields required. A caller does not have to send
    // them, so advertising them as required would be wrong — get_recipe's `mode`
    // is the clearest case.
    const { $schema, ...inputSchema } = (z as any).toJSONSchema(z.object(t.schema), {
      io: 'input',
...
/** Runs one tool by name. The single execution path for every transport. */
...
/** Counts the server card advertises, derived so they cannot go stale. */
export const MCP_STATS = {
  datasheets: COOK_TIME_DATASHEETS.length,
  appliances: SERVABLE_APPLIANCES.length,
